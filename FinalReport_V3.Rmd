...
Also for any countires who have been devastated by war, the NMC values are not available. The heat map below shows the CINC values of countires that were destroyed by both the Holocaust and the fight for freedom from Communism. 
...
The MID data is thorough and well collated. As conflicts or wars can have multiple dimentions to them, the MID data is broken down into 2 main data sets. MIDA contains an observation per conflict  with details about the start and end of the conflict, the fatalities, the outcomes, the settlements, the highest action taken etc. The other part of the dataset- MIDB, contains one obeservation per actor in a conflict, thus adding a different dimension to the data.
...
```
As seen above, most categorical variables have been neatly factored to maintain comformance in formats such as 0 being the lowest and increasing there one. This aides in better data representation. 
Also for this data, most cases of missing data, which is represented with -9 can be safely ignored. 
...
ggplot() + 
  ...
  ggtitle("Countries Initiating Most Number Of Conflicts That Year") +
  xlab("Year") +
  ylab("Count") +
  ...
  guides(fill=guide_legend(title="War Period", 
                           ...
                           nrow=1)) +
  scale_fill_manual( 
    breaks= c("WWI", "WWII", "Cold War", "Korean War", "Vietnam War", "Afghanistan War"),
    values=c("salmon", "paleturquoise3", "lightsteelblue2", "lightsteelblue4", "lightsteelblue3", "pink3", "green", "pink", "red", "orange", "grey", "blue", "salmon", "paleturquoise3", "lightsteelblue2", "lightsteelblue4", "lightsteelblue3", "pink3"))
  
```


This graph plots the most belligerent countries each year against the number of conflicts that it had initiated. The information is superimposed with different war periods. To reduce clutter, we've omitted observations below six conflicts each year. As seen above, the top initiators of conflicts had ready predisposition towards an escalation or impending war. Bearing this in mind and with the intentional goal of finding predictors, we later delved into the different dimensions of a conflict that were visible in the data before, during and after a major war was fought.
...
The data is rich and contains multifaceted information such as the Outcomes, Settlements, The Number of Fatalities, Minimum Duration, The Highest Action taken and Hostility Level during each militarized conflict in the last century. We've selected a few important variables and plotted them in a pcp to see if we can spot correlations between them.  
...
```
In the graph plotting War Time Correlations, we see that most of the datapoints for Fatality, Settlement and Outcome gravitate towards the top half. There's also a high level of correlation in there. As for the Peace Time Correlations, the first three variables seem pretty evenly distributed.

Below, we dig a little deeper into variables like Fatalities, Settlements and Outcomes of the conflicts. Each graph is overlayed with the time period of the major wars. Through this exploration, we try to spot possible predictors of a war.  
