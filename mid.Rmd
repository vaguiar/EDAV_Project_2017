...
ggplot() + 
  ...
  scale_fill_manual(values = myPal) +
  geom_rect(data=d, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2, fill=Conflict), alpha=0.15) +
  ggtitle("Militarized Disputes By Year (Settle)") 
...
ggplot() + 
  geom_bar(data=MIDA, aes(x = StYear), stat="count") +
  facet_wrap(~Outcome, nrow= 6, labeller=facet_names) +
  scale_fill_manual(values = myPal) +
  geom_rect(data=d, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2, fill=Conflict), alpha=0.15) +
  ggtitle("Militarized Disputes By Year (Outcomes)") 
