...
G4 <- c("United States of America", "United Kingdom", "Russia", "China", "France", "Canada", "Italy", "Germany")
...
```{r}
US_trade <- filter(nat_trade, ccode == 2, year>=1900)

ggplot() + 
  scale_x_continuous(name="x") + 
  scale_y_continuous(name="y") +
  geom_rect(data=d, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2, fill=Conflict), alpha=0.15) +
  geom_line(data=US_trade, aes(year, exports)) +
